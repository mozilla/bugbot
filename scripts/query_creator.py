...
def getReportURL(approval_flag, span, flag):
	a = urllib2.urlopen("https://bugzilla.mozilla.org/page.cgi?id=release_tracking_report.html&q=" + approval_flag + "%3A%2B%3A" + span + "%3A0%3Aand%3A" + flag + "-%3A")
	return a.url

def createQuery(title, short_title, url, show_summary):
	file_name = 'queries/' + str(datetime.date.today()) + '_' + short_title
...
unlanded_beta_url =	getReportURL("approval-mozilla-beta", 	cycle_span, "213")
unlanded_aurora_url =	getReportURL("approval-mozilla-aurora", cycle_span, "219")
#unlanded_esr10_url =	getReportURL("approval-mozilla-esr10",	cycle_span, "166")
unlanded_esr17_url =	getReportURL("approval-mozilla-esr17",	cycle_span, "223")
...
if weekday >= 0 and weekday < 5:
	...
	#queries.append(createQuery("Unlanded ESR10 Bugs", "unlanded_esr10", unlanded_esr10_url, False))
...
command = "python scripts/email_nag.py -t templates/daily_email -m user@mozilla.com -p pass -n "

for query in queries:
	command += " -q " + query 

command += " -s \'" + datetime.datetime.today().strftime("%A %b %d") + " -- Daily Release Tracking Alert\'"
