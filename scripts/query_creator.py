...
def createQueriesList():
    ...
    unlanded_esr17_url =	getReportURL("approval-mozilla-esr17",	cycle_span) + ";field0-0-0=cf_status_firefox" + esr_version + ";type0-0-0=nowordssubstr;value0-0-0=unaffected%2C fixed%2C verified%2C wontfix"
...
if __name__ == '__main__':
    ...
    command = [scripts_dir + "email_nag.py", "-t", "daily_email", "--no-verification", "-m", config['ldap_username'], "-p", config['ldap_password'], "-d"]
    ...
    cleanUp()
    logfilename = datetime.datetime.today().strftime("%A_%b_%d") + "-output.log"
